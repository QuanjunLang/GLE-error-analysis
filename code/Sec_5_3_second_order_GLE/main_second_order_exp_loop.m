...
ttls = {'Translation', 'Dilation', 'Cut-off', 'Oscillation'};

figure('Units','inches','Position',[1 1 9.6 2.7]); % wide figure, short height
tiledlayout(1,Num_method, 'Padding', 'compact', 'TileSpacing', 'compact');
...
for i = 1:Num_method
    ...
    upbd_lgd = sprintf('upper bound $C_4(h(t)+c)$, decay rate: %.2f', -abs(kernelInfo.h_rate));

    hold(ax,'on'); grid(ax,'on');

    if i == 1
        % Capture handles for the shared legend (generic labels)
        hExp = semilogy(t, Rt, 'LineWidth', 4, 'DisplayName', 'Expectation of Lyapunov function');
        hFit = semilogy(t, v_fit_semi, 'LineWidth', 2, 'DisplayName', 'Fitted decay line');
        hUb  = semilogy(t, C2*(ht + c2), '--', 'LineWidth', 2, 'DisplayName', upbd_lgd);
        % ylabel('Trajectory difference')
    else
        semilogy(t, Rt, 'LineWidth', 4);
        semilogy(t, v_fit_semi, 'LineWidth', 2);
        semilogy(t, C2*(ht + c2), '--', 'LineWidth', 2);
        legend(ax,'off');
    end

...
%% System settings
sysInfo.d       = 1;
sysInfo.gamma   = 50;         % Damping coefficient
...
%%
plot_trajectory_decay_fit(trajInfo{1}, sysInfo, 'test', 1)
...
% 1. Regular scale
subplot(1,3,1);
plot(t, r_square_1, 'b-', 'DisplayName', '$r_1^2$'); hold on;
plot(t, r_square_2, 'r--', 'DisplayName', '$r_2^2$');
plot(t, r_square_diff, 'k-.', 'DisplayName', '$\|x_1 - x_2\|^2$');
xlabel('Time step'); ylabel('Lyapunov value');
title('Regular scale'); legend show;
grid on;

% 2. Semilogy
subplot(1,3,2);
semilogy(t, r_square_1, 'b-', 'DisplayName', '$r_1^2$'); hold on;
semilogy(t, r_square_2, 'r--', 'DisplayName', '$r_2^2$');
semilogy(t, r_square_diff, 'k-.', 'DisplayName', '$\|x_1 - x_2\|^2$');
xlabel('Time step'); ylabel('Lyapunov value');
title('Semilogy'); legend show;
grid on;

% 3. Log-log
subplot(1,3,3);
loglog(t, r_square_1, 'b-', 'DisplayName', '$r_1^2$'); hold on;
loglog(t, r_square_2, 'r--', 'DisplayName', '$r_2^2$');
loglog(t, r_square_diff, 'k-.', 'DisplayName', '$\|x_1 - x_2\|^2$');
xlabel('Time step'); ylabel('Lyapunov value');
title('Log-log'); legend show;
grid on;
